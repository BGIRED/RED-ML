...
my $usage=<<USAGE;
    ...
    Requirements:
        ...
        simpleRepeat, downloaded from http://hgdownload.soe.ucsc.edu/goldenPath/hg38/database.
        awk '{print \$2"\\t"\$3"\\t"\$4}' simpleRepeat.txt > simpleRepeat.bed
        bedtools merge -i simpleRepeat.bed > simpleRepeat.merge.bed
        Alu, downloaded from http://hgdownload.soe.ucsc.edu/goldenPath/hg38/database.
...
my ($rnabam,$reference,$dbsnp,$simpleRepeat,$alu,$snplist,$outdir,$help);
GetOptions(
    "rnabam=s"=>\$rnabam,
...
my ($c1,$c2,$c3,$c4,$c5,$c6,$c7,$c8,$c9,$c10,$c11,$c12,$c13,$c14,$c15,$c16,$c17,$c18,$c19,$c20,$c21,$c22,$c23,$c24,$c25,$c26,$c27,$c28) = (-4.28286923,-4.78881319,-1.33047308,0.12856753,6.23200593,-0.95764449,-1.15334322,0.24150165,3.66116956,5.94310599,0.32332839,-0.13383096,-1.52799030,-0.61622349,2.18559736,0.74699106,-0.61938967,-0.01162138,-0.14203363,0.03999367,-2.27176878,0.00520151,-0.04175044,6.38014840,1.39618629,0.24702937,-12.51520056,-12.35391242);
...
while (<IN>){
    chomp;
    my ($var_reads,$fre,$mfre,$bqua,$bino,$ref_end,$ref_mid,$alt_end,$end_c,$alt_mid,$end_p,$endratio,$ref_minus,$ref_plus,$alt_minus,$alt_plus,$sb_c,$strandbias_p,$strandbiasratio,$homolen,$sr,$leftvalue,$rightvalue,$AG,$alu,$motif,$snpvalue,$alt_end_f,$sb_f,$lowfre);
    if ($_ =~ /^#/) {next;}
    my @a = split /\s+/,$_;
    my $pos = $a[0]."_".$a[1];
    ...
    print OUT1 $pos."\t".$var_reads."\t".$fre."\t".$mfre."\t".$bqua."\t".$bino."\t".$ref_end."\t".$ref_mid."\t".$alt_end."\t".$end_c."\t".$alt_mid."\t".$end_p."\t".$endratio."\t".$ref_minus."\t".$ref_plus."\t".$alt_minus."\t".$alt_plus."\t".$sb_c."\t".$strandbias_p."\t".$strandbiasratio."\t".$homolen."\t".$sr."\t".$leftvalue."\t".$rightvalue."\t".$AG."\t".$alu."\t".$motif."\t".$snpvalue."\n";
